...
function updateSpriteFrame() {
  ...
  //set background img
  catSprite.style.backgroundImage = `url('${getImageUrl(anim.file)}')`;
...
function randomWalk() {
  ...
  if (catState.isSitting) {
    ...
    if (catState.idleTimer > 300) { //5secs
      if (Math.random() < 0.3) {
        setAnimation('lying');
  ...
  //gravity simu
  if (catState.y < window.innerHeight - 140) {
    catState.velocityY += 0.5;
    catState.y += catState.velocityY;
  } else {
    catState.y = window.innerHeight - 140;
    catState.velocityY = 0;
  }
  
  //bounce off walls
  if (catState.x <= 0 || catState.x >= window.innerWidth - 128) {
    catState.velocityX *= -1;
    catState.x = Math.max(0, Math.min(window.innerWidth - 128, catState.x));
  }
  //decrease energy
...
document.addEventListener('mouseup', () => {
  if (catState.isDragging) {
    ...
    if (catState.y < window.innerHeight - 200) {
      catState.velocityY = 0;
    }
